...
def contact(request):
    ...
    if request.method == 'POST':
        ...
        if subject is not None and message is not None:
            ...
            form.save()
            messages.success(request, "We got your message. We will get back to you shortly")
...
def search_users(request):
    ...
    if query:
        users = User.objects.filter(Q(username__icontains=query)| Q(first_name__icontains=query) | Q(last_name__icontains=query)).order_by('-date_joined')
